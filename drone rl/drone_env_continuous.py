...
class DroneEnv(gym.Env):  # ✅ Inherit from gymnasium.Env
    def __init__(self):
        ...
        self.action_space = spaces.Box(low=0, high=1.0, shape=(5,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        if (action[4] > 0.5):
            self.rotor_speeds[0] = round(float(action[0]),2)
            self.rotor_speeds[1] = round(float(action[1]),2)
            self.rotor_speeds[2] = round(float(action[2]),2)
            self.rotor_speeds[3] = round(float(action[3]),2)
            # Only set rotor speeds if the fifth action value is above 0.5
            
            # Reward similar rotor speeds
            max_rotor = max(self.rotor_speeds)
            min_rotor = min(self.rotor_speeds)
            rotor_diff = max_rotor - min_rotor
            reward += (0.5 - rotor_diff)  # Reward for similar rotor speeds
        ...
        self.client.moveByMotorPWMsAsync(
            ...
            duration=0.5 / 3 # Duration for the motor command
        ).join()
        ...
        if (distance > sparseRadius + 50): # drone starts out ~81 units away from landing pad
            ...
            if (random.random() < 0.01):
                print(f"Penalty for being far from landing pad: {(distance - sparseRadius - 50)/5}")
